...
审查台支持在同一个作业任务中继续与 Agent 对话。每条修改要求和 Agent 回复都会保存在本地数据库；Agent 会复用原题、附件、课件和现有答案，而不是重新创建无关任务。每一轮都会撤销旧批准、重新生成待提交 PDF，并可通过 `submission-filename.txt` 使用用户要求的安全文件名。审查台只默认选择当前显示的待提交 PDF，后端也会拒绝批准其他未作为当前审查对象的文件。

### 自动门控

每次同步都会为作业记录一个可解释的门控结果。系统默认忽略已完成、已过期或锁定的项目，以及纸质版/线下提交、补交入口、无需提交、在线测验和不受支持的提交形式。只有支持在线文本或文件提交，并且说明中包含完整任务或可读取题目附件的作业才标记为“Agent 可处理”；只有题号、教材章节或空说明的项目会标记为“缺少题面”。

门控会分类展示已有作业，但不会因此批量运行。同步中新出现且通过门控的作业会依次进入 Agent 草稿队列；它们仍然只生成本地产物，必须经过原有人工审查才能批准，更不会绕过提交总锁。可在 `.env` 中关闭自动排队：

```dotenv
ELEARNING_AGENT_AUTO_DRAFT_ENABLED=false
```
